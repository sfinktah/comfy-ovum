...
def _oget(obj, key, default=None, call=False):
    ...
    _deep_get : uses `_oget` to traverse deeply nested objects

    Examples
    --------
    >>> import sys
    >>> _oget(sys.modules, '__main__')
    <module '__main__' (built-in)>
    >>> _oget(sys.modules['__main__'], 'oget')
    <function _oget at 0x000001A9A1920378>
    """
...
def _deep_get(obj, path, default=AttributeError):
    ...
    path : list|str
        heirachical path in form of list or dotted string
    default : any
    ...
    --------
    _dotted : creates `path` from _dotted string
    _oget : get attribute or dictionary key
    ...
    for piece in _dotted(path):
        ...
        obj = _oget(obj, tmp, AttributeError)
...
def _pluck(obj, key):
    return [_deep_get(x, key) for x in obj]
...
def _isIterateeCall(value, index, obj):
    """
...
# noinspection PyPep8Naming,PyUnusedLocal,PyShadowingNames,PyShadowingBuiltins
class underscore(object):
    ...
    def castArray(self):
        return self._wrap(asList(self.obj))
    ...
    def most(self, percent=0.9, func=lambda x, *a: not not x):
        """ Determine whether most of the elements (by percentage) match a truth test.
        """
        self.altmp = True
        ...
        total = yes + no
        if total == 0:
            return self._wrap(True)  # Handle empty collection case
        return self._wrap((yes / total) >= percent)
    ...
    def re_findall(self, pattern, flags=0):
        ...
        for ob in Array(self.obj):
            results.extend(re.findall(pattern, ob, flags))
    ...
    def zip(self, *args):
        ...
        # Create padded versions of arrays without modifying originals
        padded_args = []
        for arg in args:
            padded = list(arg)  # Create a copy
            while len(padded) < maxLen:
                padded.append(None)
            padded_args.append(padded)
        
        return self._wrap(list(zip(*padded_args)))
