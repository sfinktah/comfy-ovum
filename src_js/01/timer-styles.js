...
/* Underscore info panel styles */
.ovum-us-info {
    padding: 6px 8px;
    /* background: var(--p-content-background, rgba(0,0,0,0.35)); */
    border-top: 1px solid var(--p-content-border-color, rgba(255,255,255,0.15));
    font-size: 11px;
    color: var(--p-text-muted-color, rgba(255,255,255,0.85));
    white-space: normal;
    max-width: 100%;
...
.ovum-us-section-label {
    opacity: 0.85;
    font-weight: 600;
    color: var(--p-text-color, rgba(255,255,255,0.85));
}
.ovum-us-section-content {
    margin-top: 2px;
}
.ovum-us-code {
    margin: 4px 0 0 0;
...
export function injectTimerStyles () {
    ...
    if (document.getElementById("ovum-tailwind-style")) {
        return;
    }
    const link = document.createElement("link");
    link.id = "ovum-tailwind-style";
    link.rel = "stylesheet";
    // The CSS is built to this path by `npm run build:css` (vite --mode css)
    link.href = "/ovum/web/css/tailwind.css";
